...
inline Logspace logspace_subsmall( Logspace iX, Logspace iY ) {
  if (iX - iY > 36.7) return iX;
  return iX + log(1.0-exp(iY-iX));
}
...
inline Logspace logspace_sub( Logspace iX, Logspace iY ) {
  if (iX>iY) return logspace_subsmall(iX,iY); else { 
    cerr << "Logspace: negative result (" << iX << " - " << iY << ")" << endl; 
    return iX; 
  }
}
...
struct LogspaceMethods
{
  ...
  static inline Value pdiff( Value iX, Value iY) { if(iX>=iY) return logspace_subsmall(iX,iY); else {
    cerr << "Logspace: negative result (";
    LogspaceMethods::print(cerr,iX) << " - ";
    LogspaceMethods::print(cerr,iY) << ")" << endl;
    return iX; }}
  ...
  static inline void pdiffacc( Value& iX, Value iY) { iX = logspace_sub(iX,iY); }
...
public:
  ...
  inline friend double log( const Algebra& lx ) { return _doublelog( lx.val ); }
  inline friend Algebra exp( const Algebra& px ) { return from_log( _doubleexp( to_prob(px) ) ); }
  inline static Algebra doubleexp( double px ) { return from_log( _doubleexp( px ) ); }
...
private:
  ...
  static inline Value _doubleexp (double X) { return AlgebraMethods::doubleexp( X ); }
  static inline double _doublelog (Value X) { return AlgebraMethods::doublelog( X ); }
